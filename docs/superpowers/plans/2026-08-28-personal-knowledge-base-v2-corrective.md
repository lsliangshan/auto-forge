...
- Create `apps/desktop/resources/user-cache-migrations/0008_privacy_consent_revocation.sql`
...
- User authorization on 2026-08-29 removes the prior single-final-fix-wave restriction. Continue scoped final remediation waves until local Critical/Important findings are closed; each wave must retain RED evidence, an immutable scoped review package and independent re-review. Do not stop merely because a wave count is exhausted. Pause only for a genuine product/security decision that cannot be inferred safely.
