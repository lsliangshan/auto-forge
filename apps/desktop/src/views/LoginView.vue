  ...
  <AuthLayout>
    ...
    >
      <template v-if="method !== 'password'">
        <label :for="`login-${method}`">{{ method === 'phone' ? '手机号' : '邮箱' }}</label>
        <el-input
          :id="`login-${method}`"
          v-model="target"
    ...
    <p class="auth-switch">
      还没有云端账号？<RouterLink to="/register">
        去注册
      </RouterLink>
    </p>
  </AuthLayout>
...
async function sendCode() {
  if (method.value === 'password' || auth.sendingOtp || auth.submitting || countdown.value > 0) return
  validationError.value = ''
  auth.error = ''
...
.login-methods { display: grid; grid-template-columns: repeat(3, 1fr); gap: 4px; margin-bottom: 18px; border: 1px solid var(--af-border); border-radius: 10px; padding: 4px; background: var(--af-canvas); }
.login-methods button { border: 0; border-radius: 7px; padding: 8px 6px; background: transparent; color: var(--af-text-muted); font: inherit; font-size: 13px; cursor: pointer; }
.login-methods button.active { background: var(--af-surface); color: var(--af-graphite); font-weight: 650; }
.login-methods button:focus-visible { outline: 2px solid var(--af-cobalt); outline-offset: 1px; }
.login-methods button:disabled { cursor: not-allowed; opacity: .65; }
