...
function seedJob(database: ReturnType<typeof memoryKnowledgeStore>['database'], input: {
  ...
  if (input.activeText) {
    ...
    ) VALUES ('old', 'doc', 1, 'ready', 'old-hash', '00000000000000000000000000000001', 1, 1)`).run()
  ...
  database.prepare(`INSERT INTO document_versions(
    id, document_id, version_number, status, content_hash, object_id, created_at, publication_generation
  ) VALUES ('new', 'doc', ?, 'staging', 'new-hash', '00000000000000000000000000000002', 2, ?)`).run(input.activeText ? 2 : 1, input.generation)
