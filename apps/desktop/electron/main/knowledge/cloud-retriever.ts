...
export class CloudKnowledgeRetriever {
  ...
  async search(query: string, knowledgeBaseIds: readonly string[]) {
    ...
    if (!hasExactKeys(response, [
      ...
      || !['hybrid', 'keyword_only_consent', 'keyword_only_provider'].includes(response.strategy)
    ...
    if (response.generations.length !== knowledgeBaseIds.length
      ...
      || response.keywordCandidates.some(item => !validCandidate(
        item, generationByBase.get(item.knowledgeBaseId) ?? '',
      ))
      || response.vectorCandidates.some(item => !validCandidate(
        item, generationByBase.get(item.knowledgeBaseId) ?? '',
      ))) {
      throw { code: 'INVALID_CLOUD_RETRIEVAL_RESPONSE' }
    }
    ...
    const byId = new Map(
      [...response.keywordCandidates, ...response.vectorCandidates].map(item => [item.id, item]),
    )
    ...
    return {
      ...
      evidence: fused.map(item => byId.get(item.id)!).filter(Boolean),
    }
  }
}
