...
function toolbarDocument(
  ...
  const automation = automationBindingId === undefined
    ? ''
    : `<div class="automation-controls"><span class="automation" aria-live="polite"><span class="automation-state-dot" aria-hidden="true"></span>AI 正在操作网页</span><a class="automation-stop" href="autoforge-browser://continuation/stop/${html(automationBindingId)}">停止</a><a class="automation-takeover" href="autoforge-browser://continuation/takeover/${html(automationBindingId)}">接管</a></div>`
...
function inputShieldDocument(): string {
  return '<!doctype html><html><head><meta charset="utf-8">'
    ...
    + '<style>*{box-sizing:border-box}html,body{width:100%;height:100%;margin:0;overflow:hidden}'
    + 'body{position:relative;background:radial-gradient(circle at 50% 0,rgb(37 99 235 / 10%),transparent 32%),rgb(0 0 0 / 0.004);font:12px -apple-system,BlinkMacSystemFont,"Segoe UI","PingFang SC","Microsoft YaHei",sans-serif}'
    + 'body:before{position:fixed;inset:0;border:2px solid rgb(37 99 235 / 58%);box-shadow:inset 0 0 38px rgb(37 99 235 / 9%);content:""}'
    + 'body:after{position:fixed;top:0;left:-34%;width:34%;height:3px;background:linear-gradient(90deg,transparent,#60a5fa,transparent);content:"";animation:ai-progress 1.8s ease-in-out infinite}'
    + '.ai-operation-banner{position:fixed;top:18px;left:50%;display:grid;grid-template-columns:auto 1fr;column-gap:10px;min-width:260px;padding:10px 15px 10px 12px;border:1px solid rgb(147 197 253 / 45%);border-radius:12px;background:rgb(13 42 91 / 94%);box-shadow:0 12px 32px rgb(15 23 42 / 24%),0 0 0 1px rgb(255 255 255 / 8%) inset;color:#fff;transform:translateX(-50%);backdrop-filter:blur(12px)}'
    + '.ai-operation-dot{grid-row:1/3;align-self:center;width:10px;height:10px;border-radius:50%;background:#60a5fa;box-shadow:0 0 0 5px rgb(96 165 250 / 18%);animation:ai-pulse 1.45s ease-in-out infinite}'
    + '.ai-operation-banner strong{font-size:13px;font-weight:750;line-height:1.35;letter-spacing:.01em}.ai-operation-banner span:last-child{margin-top:2px;color:rgb(219 234 254 / 82%);font-size:10.5px;line-height:1.35}'
    + '@keyframes ai-progress{50%,100%{left:100%}}@keyframes ai-pulse{50%{opacity:.65;box-shadow:0 0 0 8px rgb(96 165 250 / 7%)}}'
    + '@media(max-width:520px){.ai-operation-banner{top:12px;min-width:0;width:calc(100% - 32px)}}'
    + '@media(prefers-reduced-motion:reduce){body:after,.ai-operation-dot{animation:none}body:after{left:0;width:100%;opacity:.72}}'
    + '</style></head><body aria-hidden="true"><div class="ai-operation-banner"><span class="ai-operation-dot"></span><strong>AI 正在操作此网页</strong><span>操作完成后可继续使用</span></div></body></html>'
}
...
class ElectronBrowserTab implements BrowserWorkspaceTab {
  ...
  open(
    ...
    return this.workspace.navigate(this.state, url, allowedOrigins, allowedUrlPatterns)
  }
...
export class ElectronBrowserWorkspace implements BrowserWorkspacePort, BrowserPageCdpPort {
  ...
  private inputShield: WebContentsViewPort | undefined
  private inputShieldCreation: Promise<void> | undefined
  private inputShieldAttached = false
  ...
  private async createTab(input: BrowserWorkspaceAcquireInput): Promise<TargetTabState> {
    ...
    contents.on('render-process-gone', () => {
      ...
      this.handleDestroyed(state, contents)
    })
    contents.on('destroyed', () => { this.handleDestroyed(state, contents) })
  ...
  private ensureInputShield(): Promise<void> {
    if (this.inputShieldCreation) return this.inputShieldCreation
    if (this.inputShield && !this.inputShield.webContents.isDestroyed()) return Promise.resolve()
    this.inputShield = undefined
    const creation = this.createInputShield()
    this.inputShieldCreation = creation
    void creation.finally(() => {
      if (this.inputShieldCreation === creation) this.inputShieldCreation = undefined
    }).catch(() => undefined)
    return creation
  }

  private async createInputShield(): Promise<void> {
    const window = this.window
    if (!window || window.isDestroyed() || this.shuttingDown) throw failure('PAGE_CLOSED')
    const shield = new this.options.WebContentsView({
      webPreferences: {
        partition: toolbarPartition,
        nodeIntegration: false,
        contextIsolation: true,
    ...
    const documentUrl = `data:text/html;charset=utf-8,${encodeURIComponent(inputShieldDocument())}`
    let loadingInitialDocument = true
    shield.setBackgroundColor('rgba(0, 0, 0, 0.004)')
    shield.webContents.setWindowOpenHandler(() => ({ action: 'deny' }))
    shield.webContents.on('will-navigate', (event: NavigationEvent, url: string) => {
      if (loadingInitialDocument && url === documentUrl) return
      event.preventDefault()
    })
    ...
    try {
      ...
      if (this.window !== window || window.isDestroyed() || this.shuttingDown
        || this.inputShield !== shield) throw failure('PAGE_CLOSED')
    } catch (error) {
      if (!shield.webContents.isDestroyed()) shield.webContents.close()
  ...
  private async restricted<T>(
    ...
    state.allowedUrlPatterns = allowedUrlPatterns ? [...allowedUrlPatterns] : undefined
    state.navigationViolation = undefined
    this.setBlockedOrigin(state, undefined)
    try {
      let result!: T
      ...
      const currentMatchesPattern = allowedUrlPatterns
        ?.some((pattern) => matchesHttpsUrlPattern(pattern, current)) ?? false
      if (current !== 'about:blank'
        && !allowedOrigins.includes(originOf(current))
        && !currentMatchesPattern) {
        throw failure('CAPABILITY_SCOPE_DENIED')
      }
      state.automationOrigins = current === 'about:blank'
        ? [...allowedOrigins]
        : allowedUrlPatterns
          ? [originOf(current)]
  ...
  private handleDestroyed(state: TargetTabState, contents: WebContentsPort): void {
    if (state.closed) return
    state.closed = true
    ...
    if (state.sessionStorageListener) {
      const listener = state.sessionStorageListener
      state.sessionStorageListener = undefined
      try { contents.debugger.removeListener('message', listener) } catch { /* renderer teardown owns final cleanup */ }
    }
    try {
  ...
  private destroyViews(): void {
    ...
    try {
      ...
      if (this.inputShield && !this.inputShield.webContents.isDestroyed()) {
