...
describe('openAppDatabase', () => {
  it('retires the shipped app-database conversion binding journal at migration 18', () => {
    const directory = mkdtempSync(join(tmpdir(), 'autoforge-retired-app-binding-'))
    temporaryDirectories.push(directory)
    const path = join(directory, 'autoforge.sqlite')
    ...
    expect(inspection.prepare(`
      ...
      WHERE type = 'table' AND name = 'conversion_block_bindings'
    `).get()).toBeUndefined()
    inspection.close()
  })
