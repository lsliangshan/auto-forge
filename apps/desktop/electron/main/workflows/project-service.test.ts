...
describe('WorkflowProjectService', () => {
  ...
  it('deletes ordinary entries but protects the required workflow files', async () => {
    ...
    await expect(projects.deleteEntry(project.id, 'src')).rejects.toMatchObject({ code: 'INVALID_INPUT' })
    await expect(projects.renameEntry(project.id, 'src', 'source')).rejects.toMatchObject({ code: 'INVALID_INPUT' })
    await expect(projects.deleteEntry(project.id, '.')).rejects.toMatchObject({ code: 'INVALID_INPUT' })
    expect(existsSync(join(root, 'workflow.json'))).toBe(true)
    expect(existsSync(join(root, 'src/index.ts'))).toBe(true)
  })

  it('rejects traversal and symlink targets for entry mutations', async () => {
    const { directory, projects } = openTestServices()
    const root = join(directory, 'project')
    const outside = join(directory, 'outside')
    writeProject(root)
    ...
    await expect(projects.createEntry(project.id, '..', 'escape.txt', 'file'))
      .rejects.toMatchObject({ code: 'PATH_OUTSIDE_PROJECT' })
    await expect(projects.renameEntry(project.id, '../outside/secret.txt', 'stolen.txt'))
      .rejects.toMatchObject({ code: 'PATH_OUTSIDE_PROJECT' })
    await expect(projects.deleteEntry(project.id, '../outside/secret.txt'))
      .rejects.toMatchObject({ code: 'PATH_OUTSIDE_PROJECT' })
    await expect(projects.createEntry(project.id, 'linked', 'escape.txt', 'file'))
      .rejects.toMatchObject({ code: 'PATH_OUTSIDE_PROJECT' })
    await expect(projects.renameEntry(project.id, 'linked', 'renamed'))
      .rejects.toMatchObject({ code: 'PATH_OUTSIDE_PROJECT' })
    await expect(projects.deleteEntry(project.id, 'linked'))
      .rejects.toMatchObject({ code: 'PATH_OUTSIDE_PROJECT' })
