...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE roles.user_id = p_caller_user_id
...
BEGIN
  ...
  PERFORM 1 FROM auth.users users WHERE users.id = p_target_user_id FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION USING MESSAGE = 'USER_NOT_FOUND', ERRCODE = 'P0001';
  END IF;
  ...
  VALUES (p_target_user_id, p_new_role, next_version, changed_at, changed_at, p_caller_user_id)
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION USING MESSAGE = 'USER_NOT_FOUND', ERRCODE = 'P0001';
  END IF;
  SELECT roles.role, roles.version INTO current_role, current_version
  FROM public.app_user_roles roles WHERE roles.user_id = p_target_user_id FOR UPDATE;
  IF NOT FOUND THEN current_role := 'user'; current_version := 0; END IF;
  IF current_role = 'super_admin' THEN
    RETURN jsonb_build_object('apply', p_apply, 'changed', false, 'userId', p_target_user_id, 'role', current_role, 'version', current_version);
  END IF;
  IF NOT p_apply THEN
    RETURN jsonb_build_object('apply', false, 'changed', false, 'userId', p_target_user_id, 'fromRole', current_role, 'toRole', 'super_admin', 'version', current_version);
  END IF;
  next_version := current_version + 1;
  INSERT INTO public.app_user_roles(user_id, role, version, created_at, updated_at, updated_by)
  VALUES (p_target_user_id, 'super_admin', next_version, changed_at, changed_at, NULL)
  ON CONFLICT (user_id) DO UPDATE SET role = 'super_admin', version = next_version, updated_at = changed_at, updated_by = NULL;
  INSERT INTO public.app_user_role_audit(
    request_id, target_user_id, old_role, new_role, operator, source,
    expected_version, result_version, created_at
  ) VALUES (p_request_id, p_target_user_id, current_role, 'super_admin', NULL, 'bootstrap', current_version, next_version, changed_at);
