...
3. 应用关键词搜索迁移 `../../migrations/20260830140000_user_admin_keyword_search.sql`；`migrations/0003_keyword_search.sql` 是与之逐字节一致的功能目录副本。
4. 为云函数配置 `AUTOFORGE_PG_RPC_BASE_URL` 与 `AUTOFORGE_PG_SERVICE_KEY`。服务密钥只能存在于云函数和受控运维环境，不得进入 Electron/Renderer。
5. 将 `function/` 作为事件型 Node.js 18.15 云函数 `autoforge-user-roles` 部署。例如，在已确认目标环境和变量后使用：
...
7. 使用明确的 CloudBase UID 预览首个超级管理员，再显式应用：
...
1. 本地测试先确认基础迁移与 `migrations/0001_user_roles.sql` 的字节和固定哈希未变化，再确认两个 additive 迁移与功能目录副本逐字节一致，并扫描回滚脚本不存在 `DROP`、`TRUNCATE` 或 `DELETE`。
2. 在隔离的 CloudBase PostgreSQL 预发布环境依次解析并执行三份迁移，验证既有角色行未变化、关键词可跨用户字段命中、entitlement 约束有效、RPC 只对 `service_role` 可执行。
...
2. 若需撤回跨字段搜索，先执行 `migrations/0003_keyword_search.rollback.sql`，它只恢复上一版用户列表函数与最小权限。
3. 执行 `migrations/0002_knowledge_entitlement.rollback.sql`。该脚本只把 `autoforge_ensure_my_role` 恢复为上一版投影并重申最小权限；它保留 `knowledge_entitlement` 列、约束和已有值。
4. 验证 RPC 不再返回 entitlement，同时数据库中的测试 entitlement 值仍存在。
5. 重新依次应用 `../../migrations/20260828200000_user_role_knowledge_entitlement.sql` 与 `../../migrations/20260830140000_user_admin_keyword_search.sql`，验证同一 entitlement 值重新出现在 RPC 投影中，再恢复依赖功能。
6. 只有在确定撤销整个用户角色 RPC 面、而非演练 additive 迁移回滚时，才可另行执行 `migrations/0001_user_roles.rollback.sql`。基础回滚同样保留角色表、审计表和已接收数据，供审计、对账与恢复。
