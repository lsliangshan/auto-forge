...
5. 只有完成迁移、私有存储、RLS、会员签名密钥、杀开关和集成测试后，才可在服务端打开 Cloud 功能。当前桌面端继续返回 `kill_switch_enabled`。
   `function/entitlement-envelope.js` 提供与桌面 Main verifier 一致的规范化 envelope。`getEntitlement` 只从可信运行时 UID 和 RPC 的严格数据库行 `{tier,status,betaEnabled,cloudEnabled,killSwitchEnabled,version,validUntil}` 构建 payload：member `active/offline_grace` 必须携带未来 `validUntil` 并签为 active，free/expired 签为 expired，unavailable 签为 revoked；free/unavailable 的空 `validUntil` 使用可信签发时刻作为终止时刻。key id、TTL 与四位年份时钟来自冻结部署配置，私钥操作仅通过注入的 KMS `signCanonical(bytes, deploymentKeyId)` 适配器；仓库不保存私钥，也不接受客户端或运行时下发的信任公钥。当前入口未配置 signer 时会返回 `ENTITLEMENT_REQUIRED`，且尚无获批公钥、KMS signer 或预发布环境，因此桌面生产信任键集保持为空。
