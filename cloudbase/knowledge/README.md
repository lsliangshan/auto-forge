...
5. 只有完成迁移、私有存储、RLS、会员签名密钥、杀开关和集成测试后，才可在服务端打开 Cloud 功能。当前桌面端继续返回 `kill_switch_enabled`。
   `function/entitlement-envelope.js` 提供与桌面 Main verifier 一致的规范化 envelope。`getEntitlement` 只从可信运行时 UID 和数据库行构建 payload，key id、TTL 与时钟来自冻结部署配置，私钥操作仅通过注入的 KMS `signCanonical(bytes, deploymentKeyId)` 适配器；仓库不保存私钥，也不接受客户端或运行时下发的信任公钥。当前入口未配置 signer 时会返回 `ENTITLEMENT_REQUIRED`，且尚无获批公钥、KMS signer 或预发布环境，因此桌面生产信任键集保持为空。
