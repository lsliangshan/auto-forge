...
1. 在隔离的 CloudBase PostgreSQL 预发布环境依次应用 `migrations/0001_personal_knowledge.sql` 和 additive `migrations/0002_personal_knowledge_workers.sql`，执行跨用户 RLS、并发发布、游标过期、作业租约和回滚演练。
...
先保持桌面 Cloud kill switch 关闭并停止 worker/云函数，依次执行 `migrations/0002_personal_knowledge_workers.rollback.sql` 和 `migrations/0001_personal_knowledge.rollback.sql`。两个回滚只撤销服务角色权限和 RPC 函数，保留表、行、RLS、复合外键与不可变约束，供审计、对账和后续恢复；禁止把删表、截断或删除已接收数据当作回滚。
