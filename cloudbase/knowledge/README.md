...
1. 在隔离的 CloudBase PostgreSQL 预发布环境依次应用 `migrations/0001_personal_knowledge.sql`、additive `migrations/0002_personal_knowledge_workers.sql`、`migrations/0003_owner_knowledge_catalog.sql` 和 `migrations/0004_cloud_job_consent_upload_recovery.sql`；将每一份与 `cloudbase/migrations/` 的对应文件作 SHA-256 字节校验后记录。确保 user-data foundation 到 `0003_privacy_consent_revocation.sql` 的完整链已应用；执行跨用户 RLS、authoritative `cloud_sync` revoke/regrant、旧授权精确重放、verified-but-unpublished 清理、并发发布、游标过期、owner catalog 分页、作业租约和回滚演练。若 consent current-state 表或当前 `cloud-sync-2026-08` acceptance 不存在，普通 Knowledge RPC 必须 fail closed。
...
6. 将本目录作为完整 CommonJS 部署包（根 `index.js` 是 worker 入口），在 Node `>=22.13.0 <27` 下使用清单锁定的 pnpm 11.15.0 执行 `pnpm install --frozen-lockfile --ignore-scripts --no-optional`，以禁安装脚本并排除 optional native addons。确认 `worker/package.json` 的 parser 依赖版本与根清单一致，再创建定时调用的 `autoforge-knowledge-worker`。它需要 `AUTOFORGE_PG_RPC_BASE_URL`、`AUTOFORGE_PG_STORAGE_BASE_URL`、`AUTOFORGE_PG_SERVICE_KEY`、`AUTOFORGE_TOKENHUB_EMBEDDING_URL`、`AUTOFORGE_TOKENHUB_API_KEY` 和严格值 `AUTOFORGE_KNOWLEDGE_MUTATION_PERMIT_PORT_VERSION=db-job-v1`；可选的 `AUTOFORGE_KNOWLEDGE_WORKER_ID` 只允许 1..96 位字母、数字、下划线或连字符。这些变量只能由服务端 secret manager 注入。TokenHub 的 embedding handler 必须在每次发送紧前调用同一 validator，mutation kind 为 `tokenhub_embedding`，并仅在验证成功的响应中回传 `mutationPermitValidated: true`。仓库不包含真实 PG Storage/TokenHub 服务实现；在两个 handler 都通过预发布验收前，目标 worker 会因 permit port 配置缺失而 fail closed，该环境变量不得仅为绕过启动检查而设置。发布包必须保留 `worker/job-process.js`、`worker/job-child.js`、`worker/parser-process.js`、`worker/parser-child.js`、`worker/settlement-process.js`、`worker/settlement-child.js`、专用 lockfile 和 PDF.js patch，并用干净的发布运行时完成真实 TXT/DOCX/PDF child 与入口 smoke。当前 hard sandbox 只批准 macOS `/usr/bin/sandbox-exec` 加 Node permission model；Linux/其他运行时会在 spawn 前 fail closed。CloudBase worker 未提供独立 UID/container、禁网 namespace、只读依赖挂载和父进程隔离前，不得部署/运行 worker 或打开 kill switch。
7. worker 每次最多领取 8 个作业，租约绑定 worker/token/expiry，仅 `TRANSIENT_FAILURE` 可在后续调度中重试，第三次失败终止。可信父调度器只负责 claim、retention 和 exact job/worker/lease/permit CAS 结算；每个已领取作业在独立的 POSIX process group/session 中运行一次性 credentialed job child，并以 TERM/KILL 整组后确认 nested parser 已退出作为结算前置。`complete_job` 与低预算 `abandon_claimed_job` 各自运行在独立可杀的一次性 settlement child；丢响应时使用原 job/worker/lease/permit capability 幂等重放并核验数据库 receipt，只有确认 outcome 后 `runOnce` 才返回。Claim 在 PostgreSQL 内由 `clock_timestamp()` 生成并存储 120 秒 mutation deadline 和不透明 permit，绑定 owner/job/worker/lease；返回的只是本地 monotonic 剩余预算和 capability。若响应到达时已不足 settlement reserve，父调度器必须用同一 worker/job/lease/permit 调用 `autoforge_knowledge_abandon_claimed_job`，回退未激活 attempt 并清空租约、permit 和 deadline。任何 SQL、Storage 或 TokenHub mutation 都不得接受 client epoch/deadline，必须在副作用紧前按 DB clock 重新验证 permit、lease 与 CAS。Embedding 每次只处理 2 个 chunk，已持久进度用 lease-CAS yield 重新排队且不消耗瞬时失败预算。Upload 依次读取并校验私有对象，再在 job child 内为每个请求启动一个无凭据、禁网、环境清空的 parser child。父调度器与 job child 只交换最多 8 KiB 输入和 1 KiB 结果帧；parser 父子只交换有界长度帧：输入最多 64 MiB，DOCX 展开总量最多 16 MiB 且压缩比最多 100，PDF 最多 1000 页、100000 个文本 item，累计文本最多 16 MiB，block/chunk 各最多 10000，索引结果最多 768 KiB（含 envelope 的响应 frame 最多 832 KiB）。PDF.js 的 Brotli 16 KiB 输出块、合并缓冲及普通 decoded-stream 增长都必须在分配前消费同一预算；文本解析不需要的 DCT/JPEG、CCITT、JBIG2、JPX direct decoder 一律拒绝。parser child 使用 128 MiB V8 old-space；job child 使用 256 MiB V8 old-space，父进程的 192 MiB parser RSS 轮询只是额外的事后 kill guard，并非 kernel/cgroup hard limit。单次 parser child wall-time 最多 119 秒；worker 为权威结算预留 5 秒，所有 RPC、Storage、parser 和 completion 共用同一 monotonic 剩余预算，早于 600 秒租约。取消、超时、崩溃、畸形或迟到 frame 都必须杀死/关闭对应 child、清零源字节并 fail closed，不能结算其他租约。若任一 mutation-capable operation 不在 reserve 内确认 abort，父调度器必须终止并确认该 job child 已退出，再用原 claim capability 做权威失败结算；父调度器本身不得 `process.abort()`。解析成功后才在同一 SQL 事务中写入 block/chunk/generation membership 并置 ready。Purge 必须先删除 `prepare` 返回的精确 Storage 集合，再提交 metadata complete。真实 Cloud worker 在没有 cgroup、rlimit 或等价 kernel memory boundary 前仍是未满足的发布门禁，必须 fail closed 且保持 kill switch 关闭。
